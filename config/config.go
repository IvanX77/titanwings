...
const DefaultLocation = "/etc/titanwings/config.yml"
...
type SystemConfiguration struct {
	// The root directory where all of the titanwings data is stored at.
	RootDirectory string `default:"/var/lib/titanwings" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other wings events are logged.
	LogDirectory string `default:"/var/log/titanwings" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/titanwings/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/titanwings/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/titanwings/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for Titan Wings installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/titanwings" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"titanwings" yaml:"username"`
...
type Configuration struct {
	...
	AppName string `default:"Titan Wings" json:"app_name" yaml:"app_name"`
...
// EnsureTitan WingsUser ensures that the Titan Wings core user exists on the
...
func EnsureTitan WingsUser() error {
	...
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("WINGS_USERNAME"), "titanwings")
	...
	log.WithField("username", _config.System.Username).Info("checking for titanwings system user")
